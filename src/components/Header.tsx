...
const Navbar = () => {
  ...
  return (
    <div
      className="h-[60px] w-full flex justify-between items-center px-3 shadow-md sticky top-0 z-10 bg-white text-sm gap-3
      lg:h-[100px] lg:px-10 lg:text-lg"
    >
      <div
        className="flex items-center gap-5 w-3/4
        lg:w-3/4"
      >
        ...
        {shouldShowSearchAndYear && (
          ...
          >
            ...
            <div className="w-full h-10 flex items-center gap-2">
              <Search className="w-[15px] lg:w-[20px]" />
              <input
                type="text"
                value={value}
                onChange={(e) => setValue(e.target.value)}
                className="w-full h-full focus:outline-none rounded-md lg:p-3"
            ...
            {pathname !== "/manage_system" && (
              <div
                className="flex justify-center items-center w-2/4 gap-1
              lg:gap-2"
              >
                <p className="text-nowrap">ปีการศึกษา</p>
                <Dropdown
                  selectedId={indexYear}
                  items={yearData}
                  onSelect={handleYearSelect} // อัปเดตค่า year
                  className="border-none text-sm"
                  classNameInput="h-[40px] border-gray-300 w-[80px] rounded-md"
                />
              </div>
            )}
          </div>
      ...
      <div className="relative w-1/4">
        <div
          className="flex items-center justify-end cursor-pointer transition duration-75 hover:text-orange gap-1
          lg:gap-2"
          onClick={toggleModal}
        >
          <p className="truncate">{user?.email ? user.email : "account"}</p>
          <ChevronDown
            className={`${
        ...
        {!user ? (
          ...
          >
            <div className="grid gap-1 pl-3 py-3 truncate">
              <div className="flex gap-2 relative">
                ...
                >
                  <PencilLine className="w-[15px] lg:w-[20px]" />
                </div>
              </div>
